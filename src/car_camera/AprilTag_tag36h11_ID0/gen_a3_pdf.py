#!/usr/bin/env python3
"""生成 A3 纸 297x300mm 停车板: 8 个 5cm 检测方框贴边。

关键点(贴边版):
  - 板子尺寸 297mm(宽) x 300mm(高): 宽度打满 A3, 高度居中。
  - AprilTag 的 8 格检测方框为 50mm，10 格模板总宽 62.5mm。
  - 标签中心距板边 25mm，因此黑色检测方框贴边；外侧静区会被裁掉。
...
# tag36h11 的 detection square 宽 8 格，模板含一格外部白色静区，
# 因此 PNG 总宽 10 格。OpenCV solvePnP 的 tag_size 对应 8 格检测方框。
TAG_BLACK_MM = 50
TAG_TILE_MM = TAG_BLACK_MM * 10 / 8
...
TAG_DIR = os.path.join(HERE, 'tags_5cm')
...
def main():
    ...
    # 输入模板包含静区，按 62.5mm 整体缩放后，内部 8/10 检测方框
    # 恰好为 50mm。
    tag_out_px = int(TAG_TILE_MM * PIX_PER_MM)
    ...
    try:
        ...
        # Pillow is already needed by most camera development environments and
        # preserves the A3 physical size through the 300 DPI resolution.
        try:
            from PIL import Image
            with Image.open(png_path) as image:
                image.save(pdf_path, 'PDF', resolution=DPI)
            print(f'\nimg2pdf unavailable ({e}); PDF saved with Pillow: {pdf_path}')
        except Exception as pillow_error:
            print(f'\nPDF failed: {pillow_error}')
    ...
    print(f'标签检测方框: {TAG_BLACK_MM}mm (务必实测黑色方框外缘)')
    print(f'完整标签模板(含静区): {TAG_TILE_MM}mm；板外一侧静区会被裁掉')
