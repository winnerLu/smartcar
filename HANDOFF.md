...
2. **防撞脱困**(和探索一起调):进度检查已由 0.5m/15s 改为 0.10m/10s。恢复树现为清图→左转→右转→激进受限倒车(0.12m@0.08m/s)→短等；正常 DWB 仍禁倒车。日志确认 BackUp 曾被 behavior_server 的 2s 内部碰撞预演在发速度前拒绝，现将 `simulate_ahead_time` 降至0.5s、局部 footprint padding 降至0.005m，并按仿真效果将局部 inflation radius 降至0.10m（全局仍为0.25m）。外层 collision_monitor 为方向感知 approach 激进兜底（TTC 0.6s，至少6个激光点触发），**待实机复测并对比 `/cmd_vel_raw` 与 `/cmd_vel_safe`**。
