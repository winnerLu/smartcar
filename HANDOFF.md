...
2. **防撞脱困**(和探索一起调):进度检查已由 0.5m/15s 改为 0.10m/10s。恢复树现为清图→左转→右转→受限倒车(0.10m@0.05m/s)→短等；正常 DWB 仍禁倒车。实测固定前后 stop/slowdown 区会把远离障碍的恢复动作也归零，已改为基于 `/local_costmap/published_footprint` 的方向感知 approach 模型（TTC 1.2s，至少4个激光点触发），**待实机复测并对比 `/cmd_vel_raw` 与 `/cmd_vel_safe`**。
